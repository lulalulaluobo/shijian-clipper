...
本地端口 80 被占用，所以把 Caddy 改到 8800：

```bash
# 临时改 compose.yaml 里的端口映射（仅本地测试，不要提交）
# 把 "80:80" 和 "443:443" 改为 "8800:80" 和 "8443:443"
```

或者用 override 文件（推荐，不动主 compose.yaml）：

```bash
cat > deploy/compose.local-ports.yaml <<'EOF'
services:
  caddy:
    ports:
      - "8800:80"
...
# 启动（带本地端口 override）
docker compose -f deploy/compose.yaml -f deploy/compose.local-ports.yaml \
  --env-file deploy/.env up -d --build 2>&1 | tail -10
```

**预期**：4 个容器 Up，`pocketbase` 显示 `healthy`。

### 1.5 健康检查

```bash
# 等约 10 秒让 PocketBase 健康
sleep 10

# 直接测 API 容器（绕过 Caddy）
docker compose -f deploy/compose.yaml --env-file deploy/.env exec -T api \
  python -c "import urllib.request; print(urllib.request.urlopen('http://localhost:8000/healthz').read().decode())"
# 预期：{"status":"ok"}

# 测 Caddy 入口（用 8800 端口）
curl -fsS -H "Host: localhost" http://localhost:8800/healthz
# 预期：{"status":"ok"}
```

如果第二个 curl 失败但第一个成功，是 Caddy 站点匹配问题。本地测试可以**直接用 API 容器的 8000 端口**，不用 Caddy。下面会演示。

### 1.6 拿到 API 端口和后端地址

```bash
# API 容器对外没暴露端口，必须从容器内访问。
# 我们用 docker exec 直接在 api 容器里跑 curl，最简单。
# 或把 API 端口临时暴露（可选，方便本机直接访问）：
...
docker compose -f deploy/compose.yaml -f deploy/compose.local-ports.yaml \
  --env-file deploy/.env up -d 2>&1 | tail -3
...
rm -f deploy/compose.local-ports.yaml  # 端口 override 文件（不要提交）
```
...
API 容器端口没暴露。检查 `deploy/compose.local-ports.yaml` 是否存在且包含 `api: ports: ["18000:8000"]`，然后 `docker compose up -d` 重启。
...
### 后端
- [ ] 4 个 Docker 容器 Up，PocketBase healthy
